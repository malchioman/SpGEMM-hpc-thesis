...
inputs=()
run_case 2 trident/run_weak_scaling.sh VARIANT=invalid
expect_runs 0
run_case 2 trident/run_weak_scaling.sh VARIANT=hybrid THREADS=2147483647
expect_runs 0
run_case 2 trident/run_weak_scaling.sh VARIANT=two_sided BINARY=/missing/trident_hybrid
expect_runs 0
hybrid_binary="$scratch/paths with spaces/trident_hybrid"
cp "$binary" "$hybrid_binary"
chmod +x "$hybrid_binary"
run_case 0 trident/run_weak_scaling.sh BINARY="$hybrid_binary" THREADS=3
expect_runs 2
read_call 1
expect_option --map-by ppr:2:node:PE=4
expect_option --threads 3
expect_option --results "$repo/results/trident/trident_hybrid/weak_scaling_v1.tsv"
for mode in strong_scaling weak_scaling local_check; do
  ...
  if [[ "$mode" == local_check ]]; then
    ...
    expect_option --bind-to none
    reject_arg --no-validate
    expect_option --results "$repo/results/tmp/trident_hybrid_local_check_v1.tsv"
  else
    expect_option --map-by ppr:2:node:PE=4
    expect_option --results "$repo/results/trident/trident_hybrid/${mode}_v1.tsv"
  fi
done
inputs=()
...
for variant in get get_pipeline; do
  ...
  expect_option --results "$repo/results/trident/trident_$variant/weak_scaling_v1.tsv"
  for mode in strong_scaling weak_scaling local_check; do
    inputs=()
    if [[ "$mode" == strong_scaling ]]; then inputs=("$matrix_a" "$matrix_b"); fi
    run_case 0 "trident/run_${mode}.sh" VARIANT="$variant" BINARY="$get_binary" THREADS=3
    expect_runs 2
    read_call 1
    ...
    if [[ "$mode" == local_check ]]; then
      ...
      expect_option --bind-to none
      reject_arg --no-validate
      expect_option --results "$repo/results/tmp/trident_${variant}_local_check_v1.tsv"
    else
      expect_option --map-by ppr:2:node:PE=3
      expect_option --results "$repo/results/trident/trident_$variant/${mode}_v1.tsv"
    fi
  done
  inputs=()
  ...
  run_case 2 trident/run_weak_scaling.sh VARIANT="$variant" BINARY="$hybrid_binary"
  expect_runs 0
  run_case 2 trident/run_local_check.sh VARIANT="$variant" BINARY="$get_binary" VALIDATE=0
  expect_runs 0
  run_case 0 trident/run_weak_scaling.sh VARIANT="$variant" BINARY="$get_binary" VALIDATE=0
  read_call 0
  expect_arg --no-validate
  run_case 17 trident/run_weak_scaling.sh VARIANT="$variant" BINARY="$get_binary" MOCK_MPI_EXIT=17
  expect_runs 1
done
